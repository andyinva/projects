...
class BibleSearchApp:
    ...
    def setup_ui(self):
        ...
        translation_frame_settings = ttk.Frame(settings_frame)
        translation_frame_settings.grid(row=0, column=3, sticky=tk.W)
        self.translation_button = ttk.Button(translation_frame_settings, text="Select Translations", 
                                           command=self.show_translation_dialog)
        self.translation_button.grid(row=0, column=0, sticky=tk.W)
        
        # Column 5: Proximity and Font Size (2 vertical)
        # Proximity
        proximity_frame = ttk.Frame(settings_frame)
        proximity_frame.grid(row=0, column=4, sticky=tk.W)
        ...
        font_frame.grid(row=1, column=4, sticky=tk.W)
        
        # Font Size button that opens dialog
        ttk.Button(font_frame, text="Font Size", command=self.open_font_size_dialog).grid(row=0, column=0, sticky=tk.W)
        ...
        # Window Heights button
        heights_frame = ttk.Frame(settings_frame)
        heights_frame.grid(row=1, column=3, sticky=tk.W)
        heights_button = ttk.Button(heights_frame, text="Adjust Heights", 
                                   command=self.show_heights_dialog)
        heights_button.grid(row=0, column=0, sticky=tk.W)
        
        
        # Exit button in top right corner of settings frame
        exit_button = ttk.Button(settings_frame, text="Exit", command=self.exit_application)
        exit_button.grid(row=0, column=5, sticky=tk.E, padx=(10, 0))
        
        # Configure settings frame to push exit button to the right
        ...
        self.selected_translations = {"KJV": True, "ASV": False, "DRB": False, "DBT": False, 
                                     ...
                                     "AKJV": False, "WNT": False}
        # Initialize translation order (1-10, default order)
        ...
        verse_display_frame = ttk.LabelFrame(main_frame, text="Bible Reading View", padding="5")
        ...
        # Create verse display text widget with scrollbar
        ...
        self.verse_display_text = tk.Text(verse_display_container, height=int(self.verse_height_var.get() * 1.5), wrap=tk.WORD, 
                                         font=('Arial', self.main_font_size_var.get()),
                                         relief="sunken", background="white")
        verse_display_scrollbar = ttk.Scrollbar(verse_display_container, orient="vertical", 
                                               command=self.verse_display_text.yview)
        self.verse_display_text.configure(yscrollcommand=verse_display_scrollbar.set)
        
        self.verse_display_text.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))
        verse_display_scrollbar.grid(row=0, column=1, sticky=(tk.N, tk.S))
        
        # Bind keyboard events for navigation
        self.verse_display_text.bind('<Key-Up>', lambda e: self.scroll_bible_up())
        self.verse_display_text.bind('<Key-Down>', lambda e: self.scroll_bible_down())
        self.verse_display_text.bind('<Button-1>', self.on_verse_display_click)
        ...
        # Update translation button text based on initial selection
    ...
    def update_font_size(self):
        ...
        if hasattr(self, 'verse_display_text'):
            self.verse_display_text.configure(font=('Arial', main_font_size))
    ...
    def update_verse_height(self):
        """Update the height of the verse display text widget"""
        if hasattr(self, 'verse_display_text'):
            height = int(self.verse_height_var.get() * 1.5)
            self.verse_display_text.configure(height=height)
    ...
    def move_to_current_subject(self):
        """Move selected search results to the subject in the dropdown"""
        if not self.selected_search_results:
            messagebox.showwarning("No Selection", "Please select verses from search results first.")
            return
        ...
        try:
            ...
            # Add selected verses
            moved_count = len(self.selected_search_results)
            for idx, result_idx in enumerate(self.selected_search_results):
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?)
                """, (subject_id, result['Reference'], result['Text'], result['Translation'], max_order + idx + 1))
            ...
            # Uncheck the moved verses in the search results treeview
...
• Gen 1:1, John 1:1, Rev 22:21 → First, middle, and last verses"""
    ...
    def apply_translation_selection(self, dialog):
        ...
        # Update both translation buttons
        self.translation_button.config(text=button_text)
    ...
    def on_search_results_select(self, event):
        ...
        try:
            ...
            if selection:
                ...
                if len(values) >= 3:  # reference, translation, text
                    reference = values[0]
                    self.log_message(f"Extracted reference: {reference}")
                    
                    # Navigate to the verse in Bible view (target verse at top)
                    self.navigate_to_verse(reference)
            else:
                # Clear verse display if no selection
    ...
    def update_verse_display(self, text):
        """Update the verse display window with the given text"""
        try:
            self.verse_display_text.config(state=tk.NORMAL)
            self.verse_display_text.delete(1.0, tk.END)
            self.verse_display_text.insert(1.0, text)
            self.verse_display_text.config(state=tk.DISABLED)
        except Exception as e:
    ...
    def display_bible_view(self):
        ...
        try:
            ...
            # Get the current translation preference
            current_translation = "KJV"  # Default
            if hasattr(self, 'translation_var'):
                try:
                    current_translation = self.translation_var.get()
            ...
            if verses:
                ...
                for i, (verse_ref, verse_text) in enumerate(verses):
                    ...
                    display_text += f"{verse_ref} {current_translation} {verse_text}\n"
                
                # Update the verse display if widget exists
                if hasattr(self, 'verse_display_text'):
                    self.verse_display_text.config(state=tk.NORMAL)
                    self.verse_display_text.delete(1.0, tk.END)
                    self.verse_display_text.insert(1.0, display_text)
                    self.verse_display_text.config(state=tk.DISABLED)
    ...
    def on_verse_display_click(self, event):
        """Handle clicks on verses in the Bible display"""
        try:
            # Get the position of the click
            index = self.verse_display_text.index(f"@{event.x},{event.y}")
            ...
            line_text = self.verse_display_text.get(line_start, line_end).strip()
            
            # Check if this line contains a verse reference (format: "Genesis 1:1 KJV text...")
            if line_text and ':' in line_text:
                # Extract just the verse reference (before the translation code)
                parts = line_text.split(' ')
                if len(parts) >= 3:  # Should have at least: book, chapter:verse, translation
                    # Find the verse reference (book + chapter:verse)
                    verse_reference = None
                    for i in range(len(parts) - 1):
                        if ':' in parts[i + 1]:  # Found chapter:verse
                            verse_reference = parts[i] + ' ' + parts[i + 1]
                            break
                    
                    if verse_reference:
                        self.log_message(f"Verse display click - navigating to: {verse_reference}")
                        self.navigate_to_verse(verse_reference)
                
        except Exception as e:
            self.log_message(f"Error handling verse display click: {e}")
