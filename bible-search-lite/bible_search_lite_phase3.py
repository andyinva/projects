...
# Import custom UI components and config from refactored modules
...
class BibleSearchProgram(QMainWindow):
    def __init__(self):
        ...
        self.title_font_size = 0  # Current: 11px
        self.verse_font_size = 0  # Current: 9px for reference and text
        self.title_font_sizes = [11, 12, 13, 14, 15]  # 5 choices, 1pt increments
        self.verse_font_sizes = [9, 10, 11, 12, 13]   # 5 choices, 1pt increments
    ...
    def setup_ui(self):
        ...
        message_section = SectionWidget("1. Message Window", self.message_label, show_settings=True, main_window=self)
    ...
    def create_reading_controls(self):
        ...
        # Navigation controls
        prev_button = QPushButton("â—€ Previous")
    ...
    def apply_font_settings(self):
        ...
        for verse_list in self.verse_lists.values():
            for verse_item in verse_list.verse_items.values():
                ...
                ref_font.setPointSize(verse_size)
                verse_item.reference_label.setFont(ref_font)

                # Update text label - same size as reference
                text_font = QFont("Arial")
                text_font.setPointSize(verse_size)
                verse_item.text_label.setFont(text_font)
    ...
    def perform_search(self):
        ...
        # Use selected translations from the translation selector
        settings.enabled_translations = self.selected_translations
        print(f"Searching translations: {settings.enabled_translations}")
    ...
    def on_search_completed(self, results):
        ...
        # Load first batch (100 results at a time)
        batch_size = 100
        initial_batch = results[:batch_size]

        # Add initial batch to the search window
        ...
        # Update message with search time
        total_results = len(self.all_search_results)
        if total_results > 0:
            # Extract search time and search term from first result
            search_time = self.all_search_results[0].get('search_time', 0) if self.all_search_results else 0
            search_term = self.all_search_results[0].get('search_term', '') if self.all_search_results else ''
            if total_results > batch_size:
                self.message_label.setText(f"({search_term}) Loaded {self.loaded_results_count} of {total_results} results in {search_time:.2f}s - scroll down for more")
            else:
                self.message_label.setText(f"({search_term}) Search completed: {total_results} results found in {search_time:.2f}s")
        else:
            self.message_label.setText("No results found")

        # Connect scroll event to load more results
        if total_results > batch_size:
            scroll_bar = self.verse_lists['search'].scroll_area.verticalScrollBar()
            scroll_bar.valueChanged.connect(self.on_search_scroll)

    def on_search_scroll(self, value):
        ...
        if value > scroll_bar.maximum() * 0.8:
            ...
            if self.loaded_results_count < len(self.all_search_results):
                ...
                print(f"Loading more results: {start_index} to {end_index}")

                # Add next batch to search window
                ...
                if self.loaded_results_count < total_results:
                    self.message_label.setText(f"({search_term}) Loaded {self.loaded_results_count} of {total_results} results - scroll down for more")
                else:
                    self.message_label.setText(f"({search_term}) All {total_results} results loaded")
    ...
    def load_context_verses(self, center_verse_id):
        ...
        # Load 50 verses starting from the clicked verse using BibleSearch
        # This will cross chapter boundaries if needed
        try:
            ...
            for i, verse in enumerate(continuous_verses):
                ...
                # Remember the first verse ID for highlighting
                if i == 0:
                    highlighted_verse_id = verse_id

            # Highlight the clicked verse (first verse in the list)
            if highlighted_verse_id and highlighted_verse_id in self.verse_lists['reading'].verse_items:
                self.verse_lists['reading'].verse_items[highlighted_verse_id].set_highlighted(True)
                # Scroll to make the highlighted verse visible at the top
                self.verse_lists['reading'].scroll_to_verse(highlighted_verse_id)

            print(f"Loaded {len(continuous_verses)} verses into reading window")
